...
> Tip: one of our precommit checks is `oxipng` which optimizes PNG images. If you don't want to use `pre-commit` locally, you can optimize your PNG images with `docker run --rm -it -v $(PWD):/workdir -w /workdir ghcr.io/shssoichiro/oxipng docs/assets/screenshots/<filename.png|jpg> --opt=4 --preserve --strip=safe`.
