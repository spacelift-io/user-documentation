...
5. Fill in the identity provider details:
    ![Add workload identity provider to GCP](<../../assets/screenshots/oidc/gcp-add-provider.png>)
      1. **Select a provider**: Select **OpenID Connect (OIDC)**.
      2. **Provider name**: Enter the email address linked to your Spacelift account.
      3. **Issuer (URL)**: The [URL of your Spacelift account](../../integrations/cloud-providers/oidc/README.md#standard-claims), including the scheme.
      4. **Audiences**: Select **Allowed audiences**, then enter the [hostname](../../integrations/cloud-providers/oidc/README.md#standard-claims) of your Spacelift account (e.g. `demo.app.spacelift.io`).
6. Fill in the provider attributes to configure mappings between Spacelift token claims (assertions) and Google attributes:
    ![GCP provider attribute mapping](<../../assets/screenshots/oidc/gcp-oidc-attribute-mapping.png>)
       1. **Google 1**: Enter `attribute.space`.
       2. **OIDC 1**: Enter `assertion.spaceId`. [Custom claims](../../integrations/cloud-providers/oidc/README.md#custom-claims) like this can be mapped to custom attributes, which need to start with the `attribute.` prefix.
...
### Step 2: Set up direct resource access
...
### Step 3: Configure access in Spacelift
