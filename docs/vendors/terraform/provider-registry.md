...
Last but not least, we assume you're going to use GoReleaser to build your provider. This is by far the most common way of managing Terraform providers which need to be available for different operating systems and architectures. If you're not familiar with GoReleaser, please refer to the [official documentation](https://goreleaser.com/){: rel="nofollow"}. You can also check out Terraform's official [`terraform-provider-scaffolding-framework` template repository](https://github.com/hashicorp/terraform-provider-scaffolding-framework){: rel="nofollow"} for an example of using GoReleaser with Terraform providers.
