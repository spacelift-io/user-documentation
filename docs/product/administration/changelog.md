...
v2.0.0 introduces some changes that prepare the way for making Spacelift more resilient to regional outages. One of these changes involves converting the single-region KMS key used to encrypt sensitive data like stack and context secrets to a multi-region key. In order to do that, the installation includes a migration to convert the data from one key to another.
...
- Converted the encryption key used for storing secrets and credentials to a multi-region key.
