...
- `stacks` containing a mapping of immutable [stack id](../../stack/README.md#stack) to the [stack configuration block](#stacks-configuration-block).
- `stack_defaults` containing the defaults common to all stacks using this source code repository. Corresponding stack-specific settings will override any stack defaults.

Setting precedence will follow this order, starting from the most important to the least important:
...
!!! info
    The precedence of the runtime configuration file changed between versions 1 and 2. Previously the stack configuration set in the Spacelift UI had a higher precedence than the stack defaults defined in the `config.yml` file.

    Starting with version 2, the values in the `config.yml` file always take precedence over stack settings defined outside the `config.yml` file.

If no stack slug is defined in the configuration file, only the first two sources are considered:
...
Runtime configuration allows flexibility that would otherwise be hard to achieve. In general, its purpose is to **preview effects of changes not related to the source code** (e.g. OpenTofu/Terraform or Pulumi version upgrades, variable changes, etc.), before they become an established part of your infrastructure.

While stack environment applies both to tracked and non-tracked branches, a runtime configuration change can be pushed to a feature branch, which triggers [proposed runs](../../run/README.md#common-run-states), allowing you to preview the changes before they have a chance to affect your state.
...
These scripts allow you to [customize the Spacelift workflow](../../stack/stack-settings.md#customizing-workflow). The following are available:
...
Project root is the path of your project directory inside the Hub repository. You can use this setting to point Spacelift to the right place if the repo contains source code for multiple stacks in various folders or serves multiple purposes like _monorepos_ combining infrastructure definitions with source code (potentially even for multiple applications).

### `runner_image` setting

!!! info
    The default runner image can be defined multiple ways.

    - No image set in runtime configuration file: Spacelift will look at the stack configuration.
    - No image set in the stack configuration: Spacelift will use the [default runner image](../../../integrations/docker.md#account-default-runner-images) specified on the account level.
    - No image set as account-level default: Spacelift will use the [**`public.ecr.aws/spacelift/runner-terraform:latest`**](https://gallery.ecr.aws/spacelift/runner-terraform){: rel="nofollow"}. See [our Docker integration documentation](../../../integrations/docker.md) for more details.
...
This setting is only valid on Terraform stacks and specifies the Terraform version that the run will use. The main use case is testing a newer version of Terraform before you use it to change the state. This version can only be equal to or higher than the one already used to apply state changes. For more details on Terraform version management, please refer to its [dedicated help section](../../../vendors/terraform/version-management.md).
...
This setting specifies the OpenTofu version to be used during the run and is only applicable to OpenTofu/Terraform stacks. It is considered when `terraform_workflow_tool` is set to `OPEN_TOFU`. Specify a version that aligns with those officially supported and tested by Spacelift.
...
- `terragrunt_tool`: Choose the tool that should be used by Terragrunt between `TERRAFORM_FOSS` (default), `OPEN_TOFU` and `MANUALLY_PROVISIONED`.
...
You must set all necessary fields because if the `terragrunt` section is present in the config, it **replaces all Terragrunt values** rather than merging them. For example, if you set `terraform_version` only, all other fields will be reset to their defaults.
