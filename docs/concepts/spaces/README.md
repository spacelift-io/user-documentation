...
Initially, you start with a `root` and a `legacy` space. The `root` space is the top-level space of your account, while the `legacy` space exists for backward compatibility with pre-spaces RBAC. [You can then create more spaces](https://spacelift-user-documentation-pr-136.onrender.com/concepts/spaces/creating-a-space.html), ending up with a big tree of segregated environments.
