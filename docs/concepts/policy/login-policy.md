...
!!! If the api key is in the "root" space and an admin key, it will not be evaluated in the login policy and will instead be treated as an account admin.  Otherwise API Keys are considered virtual users which are also evaluated with policy when login policy is turned on.  
