...
- **inbox** - allows to route messages to the Spacelift notification inbox;
...
Notification policy relies on named webhooks which can be created and managed in the [Webhooks section of Spacelift](../../integrations/webhooks.md).
...
appropriate for the message type and the `method` set as `POST`. However all that can be
changed using the webhook rule. For example if we wanted to define a completely custom request for
any [tracked run](../run/README.md) we would define a rule like this:
