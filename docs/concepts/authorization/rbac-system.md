...
fine-grained, composable approach to access control.
...
**Functional Groups**:
Organize groups by job function across the organization:
...
#### Creating custom roles using the web UI
...
!!! note "Read Access Baseline"
    The `space:read` action is required to view any subjects within a space—without it, users cannot see resources even if they have other permissions for them.

#### Creating custom roles using the terraform provider
...
Refer to [Assigning Roles to Users](../authorization/assigning-roles-users.md) for detailed instructions on how to assign roles to individual users.

### API key role bindings

Refer to [Assigning Roles to API Keys](../authorization/assigning-roles-api-keys.md) for detailed instructions on how to assign roles to API keys.

### IdP group role bindings

Refer to [Assigning Roles to Groups](../authorization/assigning-roles-groups.md) for detailed instructions on how to assign roles to IdP groups.
