...
Also, see the section on [HTTP Proxies](https://www.notion.so/Install-Guide-d9a56a01d31f441e9cd14268f089ee9f) if you need to use a proxy to allow the Spacelift components to make HTTP requests.
...
The public subnets are used to place the load balancer for the Spacelift server. If you don't need separate public and private subnets you can use the same subnets for both.
...
To install Spacelift into a custom VPC, create your VPC along with all the other required components like security groups, then edit the *custom_vpc_config.json* file. A correctly populated file will look like the following:

```
[
    "VPCID=vpc-091e6f4d35908e7c1",
    "PrivateSubnetIDs=subnet-01a25f47c5a7e94fc,subnet-035169be40fbfbbbf,subnet-09c72e8ab5499eed1",
    "PublicSubnetIDs=subnet-08aa756ab626d690f,subnet-0d03bb49f32922d93,subnet-0d85c4a80db226099",
    "DrainSecurityGroupID=sg-045061f7120343acd",
    "LoadBalancerSecurityGroupID=sg-0215aace572f21b85",
    "ServerSecurityGroupID=sg-0cb943fd285fc5c85",
    "InstallationTaskSecurityGroupID=sg-03b9b0e17cce91d3a",
    "DatabaseSecurityGroupID=sg-0b67dd8ad00e237fd",
    "AvailabilityZones=eu-west-1a,eu-west-1b,eu-west-1c"
]
```

Next, run the installer, specifying the `-v` flag to indicate you want to use a custom VPC configuration. For example:

```
./install.sh -n "spacelift.saturnhead.io" -c "arn:aws:acm:eu-west-1:123456789012:certificate/3e48a148-0281-429d-8d81-6b6cb9ffa935" -r "eu-west-1" -v
```

## HTTP Proxies

If you need to use an HTTP proxy to allow the Spacelift components to access the internet, you can specify this via the *proxy_config.json* file:
...
```
{
    "HttpProxy": "<http://my.http.proxy>",
    "HttpsProxy": "<https://my.https.proxy>",
    "NoProxy": "safe.domain"
}
...
This file will automatically be picked up during the installation process so you don't need to specify any extra parameters to the [install.sh](http://install.sh/) script.
...
## TLS serving of the server (ALB → ECS)
...
If you want the server to use HTTPS as well, you can do so by specifying the TLS certificate in `tls_config.json`. `ServerCertificateSecretsManagerArn` is the ARN of the SecretsManager secret that holds both the private and public keys of your TLS certificate in the following format:

```json
{"privateKey": "<base64-encoded-private-key>", "publicKey": "<base64-encoded-public-key>"}
```

Example `tls_config.json`:

```json
{
    "ServerCertificateSecretsManagerArn": "arn:aws:secretsmanager:eu-west-1:123456789012:secret:spacelift-server-tls-cert-123456",
    "CACertificates": []
}
...
If you use a custom certificate authority to issue TLS certs for components that Spacelift will communicate with, for example your VCS system, you need to provide your custom CA certificates. You can provide these via the `tls_config.json` file via the `CACertificates` property:
...
As well as setting up VPC endpoints, you will also need to configure an [HTTP Proxy](https://www.notion.so/Install-Guide-d9a56a01d31f441e9cd14268f089ee9f). This is required because AWS does not currently provide an endpoint for the IoT control plane, meaning that these requests cannot use a private VPC endpoint.
