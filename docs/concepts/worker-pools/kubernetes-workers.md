...
- **Worker scope**: Limits apply per individual worker, not per WorkerPool
- **Deletion safety**: Pods already being deleted are excluded from counts

!!! note "Migration from keepSuccessfulPods"
    The `keepSuccessfulPods` field has been removed in favor of the new pod history management system. If you previously used `keepSuccessfulPods: true`, set `successfulPodsHistoryLimit` to a positive value instead.
