...
- A GitHub repository where you will store your OpenTofu/Terraform configuration and a
...
resource supported by the Teleport OpenTofu/Terraform provider.
...
- Configure Spacelift to authenticate the Teleport OpenTofu/Terraform provider as a bot
...
### Configure the OpenTofu/Terraform Provider

Add the following to a file called `main.tf` to configure the Teleport OpenTofu/terraform
...
You should see an OpenTofu/Terraform plan that includes the user and role you defined
earlier:
...
When running `tofu/terraform plan`, the Teleport OpenTofu/Terraform Provider uses Machine ID
...
the run. Click **Confirm** to begin applying your OpenTofu/Terraform plan.
